  ...
  return {
    ...
    move: function(node){
      ...
      for(let i = 0; i < moves.length; i++){
        if(((node.x + moves[i][0] >= 0) && (node.x + moves[i][0] < 8)) && ((node.y + moves[i][1] >= 0) && (node.y + moves[i][1] < 8))){
    ...
    path: function (start, end) {
        ...
        do{
            let prevX = board[current[0]][current[1]].prev.x
            let prevY = board[current[0]][current[1]].prev.y
            let previousNode = [prevX, prevY]
            pathArray.push(previousNode);
            current = previousNode;
        }while(board[current[0]][current[1]].prev !== null)
            pathArray.reverse();

        if (pathArray[0][0] === start[0] && pathArray[0][1] === start[1])return pathArray;
        return[];
    },
    ...
    showBoard: ()=>{
        for(let i = 0; i < 8; i++){
            for(let j = 0; j < 8; j++){
                if(i === 0 && j === 1) continue;
                console.log(`${board[i][j].x} ${board[i][j].y} ${board[i][j].prev.x} ${board[i][j].prev.y}`)
            }
        }
    }
  };
